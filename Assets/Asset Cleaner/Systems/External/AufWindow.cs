...
namespace Asset_Cleaner {
    class AufWindow : EditorWindow {
        ...
        void OnEnable() {
            ...
            wd.Window = this;
            wd.Window.titleContent = new GUIContent("Asset Cleaner v1.50");
        ...
        void OnGUI() {
            ...
            if (store.IsProcessingInBackground) {
                var progress = store.ProcessingProgress;
                var spinnerFrame = (int) (EditorApplication.timeSinceStartup * 8) % 4;
                var spinner = new[] { "◐", "◓", "◑", "◒" }[spinnerFrame];
                EditorGUILayout.HelpBox(
                    $"{spinner} Building cache... {progress:P0}",
                    MessageType.Info);
            }
        ...
        static void Upd() {
            ...
            // Process paths incrementally
            var store = Globals<BacklinkStore>.Value;
            if (store.ProcessIncremental()) {
                Globals<WindowData>.Value?.Window?.Repaint();
            }
        ...
        void OnDisable() {
            ...
            Globals<BacklinkStore>.Value?.CancelProcessing();
